...
> **双轴 ownership**：主轴是「人人端到端的垂直切片」（草稿 §1.3），本文件定义**副轴** —— 每人额外守护一片横向领域。
> 守护人 ≠ PR 审批网关（草稿 §1.3 的禁区），守护人 = 该领域的**规范/工具/fixture 作者** + **默认 reviewer** + **问题路由节点**。
> 最近更新：2026-04-23
...
**冲突兜底**：守护人 vs slice owner 意见不一致 → 走草稿 §6 开放问题，团队共识决，不一票否决。
...
- **主要杠杆产出**：
  ...
  - `prompts/` 目录 + 版本号前缀约定
  - ADR：Citations API vs 自研 PDF 分块（草稿 §4.1）
  - Model/token 成本基线文档
