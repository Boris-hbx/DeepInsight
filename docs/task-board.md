...
> DeepInsight 采用「人人端到端」协作模式（见草稿 §1.3），**没有中央 PM**。任何人都可以发令，包括对自己发令（"认领"一条任务）。横向规范（reliability/test/SE）通过 skill、spec 模板、CI hook 落地，不当审批网关。
...
- **接令**：@阿伟 / @All / 未分配
- **关联**：spec/NNN-xxx / ADR-NNNN / 草稿 §x.y / 无
...
- **发起**：无（自认领）
- **接令**：@阿锋
- **关联**：草稿 §3 轨道 A A3、§4.1 Citations API 建议
- **依赖**：T-005、T-006
- **优先级**：P1
- **状态**：🔴 待接令
...
`create-next-app` 起步，TS 严格模式、Tailwind、shadcn/ui 初始化；落 `.eslintrc` / `prettier` / `tsconfig`。`.env.local.example` 列出 `ANTHROPIC_API_KEY`（真实 key 禁止提交，见草稿 §5）。
...
spec 模板，字段：背景、方案、非目标、**可靠性 checklist**（失败面、超时、retry）、**测试 checklist**（unit/integration/eval）、rollout、open questions。产出将被所有后续 spec 继承，是横向影响力的主要杠杆点（草稿 §1.3）。
